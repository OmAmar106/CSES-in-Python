...
void djikstra(vector<vector<pi>> &d, int &k){
    
    vector<vi> dist(len(d));
    dist[0].pb(0);
    ...
    while(!pq.empty()){
        ...
        if(dis<=dist[cur].back()){
            for(auto &[v,w]:d[cur]){
                ...
                else if(dist[v].back()>dis+w){
                    dist[v][len(dist[v])-1] = dis+w;
                    st(dist[v]);
                    pq.push(make_pair(dis+w,v));
                }
            }
