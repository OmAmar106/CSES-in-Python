...
def solve():
    ...
    for _ in range(m):
        a,b,c = list(map(lambda x:int(x)-1, sys.stdin.readline().split()))
        c += 1
        d1[a].append((b,c))
        d1[b].append((a,c))
    ...
    for i in range(n):
        ...
        while H:
            d,cur = heappop(H)
            if dist[cur]==d:
                for j,weight in d1[cur]:
                    if dist[cur]+weight<dist[j]:
                        dist[j] = dist[cur]+weight
                        heappush(H,(dist[j],j))
        df.append(dist)
    
    for _ in range(q):
        a,b = list(map(lambda x:int(x)-1, sys.stdin.readline().split()))
        print(df[a][b] if (df[a][b]!=float('inf')) else -1)
