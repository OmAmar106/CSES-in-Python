...
def solve():
    ...
    dirs = ((x1*y1,x2*y2) for x1,x2 in dirs for y1,y2 in dirs1)
    dirs = tuple(dirs)

    def ispossible(x,y):
        return 0<=x<n and 0<=y<n and ans[x][y]==-1

    for x,y,dist in bfs:
        # print(x,y,dirs)
        for dx,dy in dirs:
            # print(x,y,x+dx,y+dy,ispossible(x+dx,y+dy))
            if ispossible(x+dx,y+dy):
                ans[x+dx][y+dy] = dist+1
                bfs.append((x+dx,y+dy,dist+1))
