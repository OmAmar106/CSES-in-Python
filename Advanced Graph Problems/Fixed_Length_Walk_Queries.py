...
def extras():
    # use Decimal(number) for context to work
    # slows down the code significantly
    # by default, there is precision of 10-6
    getcontext().prec = 50
    # more precision, use Fraction
...
def reduce_time():
    # reduces run time, but increases the chance of mle
    import gc
    gc.disable()
# reduce_time()
...
def solve():
    ...
    for i in range(n):
        ...
        for j,dist2 in bfs:
            for x in d[j]:
                if dist[i*n*2+2*x+1-(dist2%2)]==float('inf'):
                    dist[i*n*2+2*x+1-(dist2%2)] = dist2+1
                    bfs.append((x,dist2+1))

    for i in range(q):
        x,y,ln = LII_1()
        x,y = min(x,y),max(x,y)
        print("YES" if dist[x*n*2+2*y+1-(ln%2)]<=ln+1 else "NO")
