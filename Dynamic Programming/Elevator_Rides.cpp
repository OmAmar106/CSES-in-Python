...
void solve() {
    ...
    rangea(i,1,1<<n){
        dp.emplace_back(1e9,1e9);
        for(int j=0;j<n;j++){
            if(i&(1<<j)){
                if(dp[i].f>dp[i^(1<<j)].f+1*(dp[i^(1<<j)].s+a[j]>x)){
                    dp[i].f = dp[i^(1<<j)].f+1*(dp[i^(1<<j)].s+a[j]>x);
                    dp[i].s = max(a[j],(dp[i^(1<<j)].s+a[j])%(x+1));
                }
                else if(dp[i].f==dp[i^(1<<j)].f+1*(dp[i^(1<<j)].s+a[j]>x) && dp[i].s>max(a[j],(dp[i^(1<<j)].s+a[j])%(x+1))){
                    dp[i].s = max(a[j],(dp[i^(1<<j)].s+a[j])%(x+1));
                }
            }
        }
    ...
    // print(dp[2]);
    print(dp[len(dp)-1].f + 1*(dp[len(dp)-1].s!=0));
