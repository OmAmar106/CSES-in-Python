...
def solve():
    ...
    for i in range(20):
        jump.append([jump[-1][jump[-1][j]] if jump[-1][j]!=2*n else 2*n for j in range(len(jump[0]))])

    fans = n
    for i in range(n):
        # i is my starting point
        ans = 0
        cur = i
        ...
        fans = min(fans,ans+(1 if cur!=i+n else 0))
