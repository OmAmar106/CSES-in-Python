...
SLI = lambda : list(map(lambda x:ord(x)-65,sys.stdin.readline().strip()))
    ...
    for j in range(n):
        ...
        for i in range(n):
            ...
            if x==y:
                ...
                newheight[i] = 1
            if i-1>=0 and x!=z:
                # st = [(i-1,0),(i,newheight[i])]
                st.clear()
                st.append((i-1,0))
                st.append((i,newheight[i]))
                cost = newheight[i]
                ans[x] += cost
                continue
            while len(st)>1 and st[-1][1]>=newheight[i]:
                x1,ht = st.pop()
                cost -= (x1-st[-1][0])*ht
            cost += newheight[i]*(i-st[-1][0])
            ans[x] += cost
            st.append((i,newheight[i]))
