...
def solve():
    ...
    for i in range(n-1,-1,-1):
        newdp = [[float('inf')]*k for i in range(n)]
        for j in range(n-1,-1,-1):
            newdp[j][ord(st[i][j])-ord('A')] = 0
            for l in range(k):
                if i+1<n:newdp[j][l] = min(newdp[j][l],1+dp[j][l])
                if j+1<n:newdp[j][l] = min(newdp[j][l],1+newdp[j+1][l])
                if i+1<n and j+1<n:newdp[j][l] = min(newdp[j][l],1+dp[j+1][l])
            ans += max(0,n-max(i,j)-max(newdp[j]))
            # print(i,j,ans)
        dp = newdp
