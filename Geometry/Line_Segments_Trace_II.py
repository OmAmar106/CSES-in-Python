...
class SegmentTree:
    def __init__(self, n):
        self.n = n
        self.tree = [LiChao(0,n) for i in range(4*self.n)]
    def update(self, idx, l, r, ql, qr, m, c):
        stack = [(idx, l, r)]
        while stack:
            idx, l, r = stack.pop()
            if r < ql or l > qr:
                continue
            if ql <= l and r <= qr:
                self.tree[idx].add_line(m, c)
                continue
            mid = (l + r) // 2
            stack.append((2*idx+1, mid+1, r))
            stack.append((2*idx, l, mid))
    def query(self, idx, l, r, pos):
        # [l,r]
        ans = -1
        while l<r:
            mid = (l + r)>>1
            ans = max(ans,self.tree[idx].query(pos))
            if pos>mid:
                l = mid+1
                idx *= 2
                idx += 1
...
def solve():
    ...
    print(*[seg.query(1,0,m,i) for i in range(m+1)])
