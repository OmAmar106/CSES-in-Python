...
def solve():
    ...
    for i in range(sqrt(n)+1):
        for j in range(sqrt(n-i*i)):
            for k in range(sqrt(n-i*i-j*j)):
                if issquare(n-i*i-j*j-k*k)[0]:
                    print(i*ans,j*ans,k*ans,issquare(n-i*i-j*j-k*k)[1]*ans)
                    return
