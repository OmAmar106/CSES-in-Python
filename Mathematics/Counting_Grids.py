...
def solve():
    ...
    # Burnside lemma -> if you are to tell distinct elements among all possible rotations
    # tell the sum of distinct possibilities for each rotation, such that all rotation of that angle are same
    # tell their sum / n

    ans = pow(2,n*n,MOD)
    ans += 2*pow(2,(n//2)*(n//2)+(n//2)*(n%2),MOD)*(pow(2,n%2,MOD))
    ans += pow(2,((n//2)*(n//2)+(n//2)*(n//2)+(n//2)*(n%2)),MOD)*(pow(2,(n//2+1)*(n%2),MOD))
    print(((ans)*pow(4,-1,MOD))%MOD)
