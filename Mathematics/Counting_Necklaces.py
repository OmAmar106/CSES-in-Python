...
def exact(n,k):
    # n raised to k such that it is not repeating any pattern of its divisor length 
    if n==1:
        return k
    # ans = pow(n,k,MOD)
    # # print(n,k)
...
def solve():
    ...
    # # kitne same kr skta hu mai voh minus krna hain 
    # # print(ans)
    # i = 1
    # while i*i<=n:
    #     if (n%i==0):
    ...
    #         # but isme ab im continuing two times its divisors

    #     i += 1

    # print(ans)

    print(sum(pow(m,gcd(i,n),MOD) for i in range(1,n+1))*pow(n,-1,MOD)%MOD)

    # Should try to think from the POV of colors rather than beads :(
