...
class Treap:
    ...
    @staticmethod
    def sum(node):
        return node.sm if node else 0
    @staticmethod
    def _update(node):
        if node:
            node.size = 1 + Treap._size(node.left) + Treap._size(node.right)
            node.sm = node.val+Treap.sum(node.left)+Treap.sum(node.right)
    @staticmethod
...
def solve():
    ...
    tp = Treap(list(map(int, sys.stdin.readline().split())))
    for i in range(m):
        t,l,r = list(map(lambda x:int(x)-1, sys.stdin.readline().split()))
        if t==0:
            node1,node2 = tp.split(l)
            node3,node4 = node2.split(r-l+1)
            node3.reverse()
            tp = Treap.merge(node1,Treap.merge(node3,node4))
        else:
            node1,node2 = tp.split(l)
            node3,node4 = node2.split(r-l+1)
            print(node3.root.sm)
