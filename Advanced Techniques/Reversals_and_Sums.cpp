...
struct Node {
    int val, pr, sz,rev,sm;
    Node *l, *r;
    Node(int v) : val(v), pr(rng()), sz(1), l(nullptr), r(nullptr),rev(0),sm(v) {}
};

int sz(Node* t) { return t ? t->sz : 0; }

int sm(Node* t){return t?t->sm:0;}

void upd(Node* t) {
    if (t) t->sz = 1 + sz(t->l) + sz(t->r);
    if (t) t->sm = sm(t->l)+sm(t->r)+t->val;
}
...
void solve() {
    ...
    Node* st1 = new Node(val);
    for(int i=1;i<n;i++){
        cin>>val;
        Node* st2 = new Node(val);
        st1 = merge(st1,st2);
        // print(st1->sm);
    }
