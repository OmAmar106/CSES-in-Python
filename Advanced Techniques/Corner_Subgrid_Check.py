...
SLI = lambda : list(map(lambda x:ord(x)-ord('A'),sys.stdin.readline().strip()))
...
def solve():
    ...
    L = [[BitSet(n) for i in range(n)] for j in range(k)]
    ispos = [False]*k

    for i in range(n):
        st = SLI()
        prev = [-1]*26
        cur = [BitSet(n) for i in range(k)]
        for j in range(n):
            if ispos[st[j]]:
                continue
            if (L[st[j]][j]&cur[st[j]]):
                # print(L[st[j]][j])
                # print((L[st[j]][j]&cur[st[j]]))
                ispos[st[j]] = True
            else:
                if prev[st[j]]!=-1:
                    L[st[j]][j][prev[st[j]]] = 1
                prev[st[j]] = j
                cur[st[j]][j] = 1
