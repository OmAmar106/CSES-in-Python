...
class MCMF:
    ...
    def min_cost(self, s, t, maxf=float('inf')):
        ...
        for u in range(self.n):
            for v, cap, _, rev, orig in self.g[u]:
                if v == t and orig > 0 and cap != 0:
                    return 1, 1
...
def solve():
    ...
    for i in range(len(L)):
        mcmf.add_edge(0,i+1,L[i],0)
    
    for i in range(len(L1)):
        mcmf.add_edge(n+i+1,2*n+1,L1[i],0)
    
    for i in range(n):
        for j in range(n):
            mcmf.add_edge(i+1,n+j+1,1,-L2[i][j])

    ans = (-mcmf.min_cost(0,2*n+1)[1])
