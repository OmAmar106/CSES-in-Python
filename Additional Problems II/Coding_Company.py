...
def solve():
    ...
    n1 = n//2
    dp = [[0]*(n1+1) for j in range(x1+1)]
    dp[0][0] = 1
    newdp = [[0]*(n1+1) for j in range(x1+1)]
    
    for i in range(len(L)):
        for j in range(x1+1):
            for k in range(n1+1):
                # print(newdp[j][k])
                newdp[j][k] = (k+1)*dp[j][k]
                if j-L[i]>=0 and k-1>=0:
                    newdp[j][k] += k*dp[j-L[i]][k-1]
                if j+L[i]<=x1 and k+1<=n1:
                    newdp[j][k] += dp[j+L[i]][k+1]
                newdp[j][k] %= MOD
        dp,newdp = newdp,dp
    
    ans = 0
    for i in range(min(x1,x)+1):
        ans += dp[i][0]
