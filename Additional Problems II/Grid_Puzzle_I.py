...
def solve():
    ...
    for i in range(len(L)):
        for j in range(len(L)):
            dnc.add_edge(i+1,j+n+1,1)
    
    for i in range(len(L)):
        dnc.add_edge(0,i+1,L[i])
    
    for i in range(len(L1)):
        dnc.add_edge(i+n+1,2*n+1,L1[i])
    
    dnc.calc(0,2*n+1)
    
    ans = [['.']*n for i in range(n)]

    for x,y in dnc.get_paths(0,2*n+1):
        ans[y[1][0]-1][y[1][1]-n-1] = 'X'
        L[y[1][0]-1] -= 1
        L1[y[1][1]-n-1] -= 1
    
    if L.count(0)!=len(L) or L1.count(0)!=len(L1):
        print(-1)
        return
