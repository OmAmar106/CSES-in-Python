...
def solve():
    ...
    seg = SegmentTree([L1[i]-(i+1) for i in range(len(L1))])
    seg1 = SegmentTree([L1[i]+(i+1) for i in range(len(L1))])
    for i in range(q):
        L = list(map(int, sys.stdin.readline().split()))
        if L[0]==2:
            print(min(seg.query(0,L[1])+L[1],seg1.query(L[1],len(L1))-L[1]))
        else:
            seg.update(L[1]-1,L[2]-L[1])
            seg1.update(L[1]-1,L[2]+L[1])
